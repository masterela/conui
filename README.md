...
at a time: `app`, `canvas`, `frame`, `layout`, `state` and `theme` carry `#[warn(missing_docs)]` and are
done, `typography`, `view` and `widget` are not yet. A module with the attribute cannot regress,
